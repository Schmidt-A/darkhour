...
if ( GetLocalInt(oTarget, "Poisoned") != TRUE)
{
SetLocalInt(oTarget, "Poisoned", TRUE);
DelayCommand(RoundsToSeconds(10), DeleteLocalInt(oTarget, "Poisoned"));
effect eAB = EffectAttackDecrease(4);
effect eDam = EffectDamageDecrease(4, DAMAGE_TYPE_DIVINE);
effect eSpeed = EffectMovementSpeedDecrease(30);
effect eAC = EffectACDecrease(4, AC_DODGE_BONUS);
effect ePoison = EffectLinkEffects(eAB, eDam);
ePoison = EffectLinkEffects(ePoison, eSpeed);
ePoison = EffectLinkEffects(ePoison, eAC);
ApplyEffectToObject(DURATION_TYPE_TEMPORARY, ePoison, oTarget, RoundsToSeconds(10));
