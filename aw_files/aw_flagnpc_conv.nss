//this is the script of the PILLARS on click
...
else
{
 ...
 if (!GetIsPC(oPC) || (GetDeity(oPC) == "GM") || GetIsDM(oPC) || (nPCTeam == TEAM_NONE))
   ...
   //if doesn't have the flag, return //
 ...
 if (nPCTeam == nFlagTeam) //if crossing our own flagstand
    ...
    else //enemy crossing  flagstand
    {
        //they click to get in line for capping
        // if the flag is not hold by the NPC
        ...
        else   // THE FLAG IS THERE
        {
        //check for people in range
            if(ExecuteScriptAndReturnInt("enemiesinrange", oPC) == FALSE) // THERE IS NOBODY IN RANGE
            {
                FloatingTextStringOnCreature("You can't steal the flag because no-one is in your range!",oPC,FALSE);
                return;
            }
            RemoveAllEffects(oEnemyFlagHolder);
