...
if(iLevel < 3)
    {
    object oBoss = CreateObject(OBJECT_TYPE_CREATURE, "lesserashdemon", lLoc, TRUE);
    SetLocalInt(oSpawn, "canspawn", 1);
    DelayCommand(14390.0, DestroyObject(oBoss));
    DelayCommand(714400.0, SetLocalInt(OBJECT_SELF, "canspawn", 0));
    return;
    }else
        {
        object oBoss = CreateObject(OBJECT_TYPE_CREATURE, "thefoglord002", lLoc, TRUE);
        SetLocalInt(oSpawn, "canspawn", 1);
        DelayCommand(14390.0, DestroyObject(oBoss));
