...
int LEISURE_THRESHHOLD =100; // * This is the number when NPCs will stop using Leisure objects
int EVENT_NEEDMOREPATRONS = 5000; // * event number when one HUB asks another for extra patrons
int  EVENT_IT_SHOUTS = 5001;
int  EVENT_NOTIT_SHOUTS = 5002;
int EVENT_IAMTHEHUNTER = 5003; // * tells hunter to broadcast its hunter message every second

int LISTEN_MOVETOTOWNHUB = 999;// * Listen pattern to get people moving towards a
                                // * townhub
int LISTEN_SOMEONEISIT = 998;   // * IT shouts this every heartbeat
int  LISTEN_IHUNTYOU = 997; // * hunters say this to scare prey
int LISTEN_IAMNOTIT = 996; // * if I am not it, I tell it this, so it comes and get me
int LISTEN_GOAWAY = 995; // * stray dog goes away if told to
...
float fDelayBetweenBarmaidAskingForDrinks = 60.0;
float FPATRONDELAY = 40.0; // * delay between patrons spawning in when others leave
float DISTANCE_TALKHUB = 4.0;

float DISTANCE_OBJECT_TALK = 10.0; // * Distance that objects will request works


int JOB_BARMAID = 1000;  ////////////////////////////////
...
int JOB_PATRON = 1003;   ////////////////////////////////
                         ...
                         ////////////////////////////////
int JOB_TAG_NOTIT = 1004;
int JOB_TAG_IT = 1005;
int JOB_FRIEND = 1006;

int JOB_WANDER = 1007;
int JOB_HUNTER = 1008; // Same job as WANDER but used to differentiate them
...
int TASK_WORK_CLEANING = 1000;    // Objects may request cleaning
int TASK_WORK_ANVIL = 1001;       // Objects may request anvil'ing
int TASK_WORK_LAMPPOST = 1002;    // lampposts want to be turned on/off
int TASK_LEISURE_READBOOK = 1003; // read a book if not busy
int TASK_LEISURE_SIT = 1004;      // sit on the chair
