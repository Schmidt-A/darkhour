...
if (nRound >= 999)
{
if (GetLocalInt(OBJECT_SELF, "Jump") != TRUE)
{
SetLocalInt(OBJECT_SELF, "Caught", TRUE);
DelayCommand(RoundsToSeconds(2), DeleteLocalInt(OBJECT_SELF, "Jump"));
FloatingTextStringOnCreature("You may jump again in 12 seconds.", OBJECT_SELF);
location lLocation = GetSpellTargetLocation();
int nDice = d100(2);
effect eVis = EffectVisualEffect(VFX_IMP_DUST_EXPLOSION);
effect eDam = EffectDamage(nDice, DAMAGE_TYPE_DIVINE);
effect eJump = EffectDisappearAppear(lLocation);
AssignCommand(OBJECT_SELF, ActionJumpToLocation(lLocation));
DelayCommand(0.5, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lLocation));
    object oCaught = GetFirstObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_LARGE, lLocation);
    while(GetIsObjectValid(oCaught))
    {
        if (oCaught != OBJECT_SELF)
        {
            DelayCommand(0.5, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oCaught));
            oCaught = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_COLOSSAL, lLocation);
        }
    }
}
}
