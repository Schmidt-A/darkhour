...
void DoMissileStorm(int nD6Dice, int nCap, int nSpell, int nMIRV = VFX_IMP_MIRV, int nVIS = VFX_IMP_MAGBLUE, int nDAMAGETYPE = DAMAGE_TYPE_MAGICAL, int nONEHIT = FALSE, int nReflexSave = FALSE)
{
    ...
    int nCasterLvl = GetCasterLevel(OBJECT_SELF);
    int nCharacterLvl = GetHitDice(OBJECT_SELF);
//    int nDamage = 0;
    int nMetaMagic = GetMetaMagicFeat();
    int nCnt = 1;
    ...
    if ( nMissiles > nCharacterLvl / 4)
        nMissiles = nCharacterLvl / 4;
    ...
    //Cycle through the targets within the spell shape until an invalid object is captured.
        ...
        while (GetIsObjectValid(oTarget) )
        {
         ...
         if (nDAMAGETYPE == DAMAGE_TYPE_MAGICAL) nEnemies = 1;
                    ...
                    if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
                    {
                        for (i=1; i <= nExtraMissiles + nRemainder; i++)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    int nIDC = GetSpellSaveDC();
                                    if (WillSave(oTarget, nIDC) == 1 )
                                    {
                                        nDam = nDam*2/3;
                                    }
                                }
