...
// End of Spell Cast Hook
    ...
    int nDoorDC =  GetLockUnlockDC(oTarget);
    ...
    int nRoll =   d20();
    int nSpellDC = 10 + GetCasterLevel(OBJECT_SELF)+ nAbilityMod + nRoll;
    ...
    while(GetIsObjectValid(oTarget))
    {
        ...
        {
            SendMessageToPC(OBJECT_SELF, IntToString(nRoll) + " (Roll) + " + IntToString(nAbilityMod) + " (Ability Modifier) +Caster Level, versus DC " + IntToString(nDoorDC));

            nResist =  GetDoorFlag(oTarget,DOOR_FLAG_RESIST_KNOCK);
            if (nResist == 0 || nSpellDC >= nDoorDC)
            {
