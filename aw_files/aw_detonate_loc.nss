...
void Knock_Down(location lLoc)
{
 ...
 int nDC = (GetFortitudeSavingThrow(oUser)/3)*2;
 while (GetIsObjectValid(oPC))
       {
       if (GetPlayerTeam(oPC) == nTeam)
         return;
         ///save VS Knock Down ///
         if (!MySavingThrow(SAVING_THROW_FORT,oPC, nDC))
           {
           int iKnock = d10();
...
{   int nTeam = GetLocalInt(OBJECT_SELF, "nTeam");
    ...
    while (GetIsObjectValid(oTarget))
          {
           if (GetPlayerTeam(oTarget) == nTeam)   return;
...
void Do_Damage(location lLoc)
{
    ...
    while (GetIsObjectValid(oTarget))
    {
        if (GetPlayerTeam(oTarget) == nTeam)
           {
           return;
           }
           else
