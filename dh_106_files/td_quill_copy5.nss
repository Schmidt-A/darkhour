...
void main()
{
    ...
    if(oOriginalBook != OBJECT_INVALID)
    {
        ...
        if(sResRef == "td_it_blankbook" && sBookName != "Blank Book")
            ...
            DelayCommand(0.1, SetDescription(CopyItemAndModify(oOriginalBook,ITEM_APPR_TYPE_SIMPLE_MODEL,0,2,TRUE),sBookstory,TRUE));
            DelayCommand(0.2, SetDescription(CopyItemAndModify(oOriginalBook,ITEM_APPR_TYPE_SIMPLE_MODEL,0,2,TRUE),sBookstory,TRUE));
            DelayCommand(0.3, SetDescription(CopyItemAndModify(oOriginalBook,ITEM_APPR_TYPE_SIMPLE_MODEL,0,2,TRUE),sBookstory,TRUE));
            DelayCommand(0.4, SetDescription(CopyItemAndModify(oOriginalBook,ITEM_APPR_TYPE_SIMPLE_MODEL,0,2,TRUE),sBookstory,TRUE));
            }
    }
    SetLocalInt(oCrafting, "crafting", iAmount - 250);
}
