    ...
    if (GetIsObjectValid(oTarget) == TRUE)
    {
        ...
        if (nTouch > 0)
        {
            ...
            if (nDamage > 0)
            {
                ...
                //Antiworld Immmunity check
                if(GetIsImmune(oTarget, IMMUNITY_TYPE_DEATH) == FALSE)
                {

                    //Antiworld uses 10 + AA level + CHAmod + INTmod as DC
                    int nDC = 14 + GetLevelByClass(CLASS_TYPE_ARCANE_ARCHER);
                    ...
                    // * if target fails a fortitude save they die
