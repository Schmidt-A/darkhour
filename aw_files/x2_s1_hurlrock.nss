...
void RockDamageHunt(location lImpact);
void RockDamageHunt(location lImpact)
{
    float fDelay;
    int nDamage;
    ...
    while (GetIsObjectValid(oTarget))
    {
        ...
        if (spellsIsTarget(oTarget,SPELL_TARGET_STANDARDHOSTILE,OBJECT_SELF))
        {
            ...
            //Roll damage for each target, but doors are always killed
             ...
             nDamage = d10(nDice);
                eDam = EffectDamage(nDamage, DAMAGE_TYPE_DIVINE, DAMAGE_POWER_PLUS_ONE);
                // Apply effects to the currently selected target.
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eDam, oTarget));
                //This visual effect is applied to the target object not the location as above.  This visual effect
                //represents the flame that erupts on the target not on the ground.
                //DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
        int nChance = d100(1);
        if ((nChance > 80) && (GetLocalInt(oTarget, "HuntKD") != TRUE) )
        {
            effect eKD = EffectKnockdown();
            DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eKD ,oTarget, RoundsToSeconds(2)));
            SetLocalInt(oTarget, "HuntKD", TRUE);
            DelayCommand(RoundsToSeconds(24), DeleteLocalInt(oTarget, "HuntKD"));
            FloatingTextStringOnCreature("You knocked down the target! Knockdown won't work on this target again for 24 seconds.", OBJECT_SELF);
        }
        }
        //Select the next target within the spell shape.
        oTarget = GetNextObjectInShape(SHAPE_SPHERE, RADIUS_SIZE_HUGE, lImpact, TRUE, OBJECT_TYPE_CREATURE | OBJECT_TYPE_DOOR | OBJECT_TYPE_PLACEABLE);
