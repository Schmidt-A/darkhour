...
/*
    ...
    Epic Progression
    Every five levels past 5 (10, 15 et cetera) an additional +2
    on attack rolls, damage and saving throws is added. As well the damage
    reduction increases by 2 and the damage power required to penetrate
    damage reduction raises by +1 (to a maximum of /+5). So at level 10, the Divine Champion would
    get +5 on attacks, damages, and saving throws. They would also get damage
    reduction 7/2.
...
void main()
{
    ...
    else if (nLevelB >5 )
    {
        nDmgRedP = DAMAGE_POWER_PLUS_FIVE;
        nDmgRedB = 6*5;
        nDamageB = DAMAGE_BONUS_12;
    }
    else if (nLevelB >4 )
    {
        nDmgRedP = DAMAGE_POWER_PLUS_FIVE;
        nDmgRedB = 5*5;
        nDamageB = DAMAGE_BONUS_10;
    }
    else if (nLevelB >3)
    {
