...
void main()
{
  ...
  if (nRow ==0)
  {
   ...
   int bgLvls = GetLevelByClass(CLASS_TYPE_BLACKGUARD,oPC);
   int assLvls = GetLevelByClass(CLASS_TYPE_ASSASSIN,oPC);
   int rogLvls = GetLevelByClass(CLASS_TYPE_ROGUE,oPC);

   int combined = (bgLvls + assLvls + rogLvls )/NO_LVLS_PER_ROUND;
   if (!(combined > 0)) combined = 1;
   nDuration =  18 * combined;
