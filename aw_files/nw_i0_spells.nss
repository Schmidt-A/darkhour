...
void TrapDoElectricalDamage(int ngDamageMaster, int nSaveDC, int nSecondary, object oCreator);
...
//:://////////////////////////////////////////////
//:://////////////////////////////////////////////
//:: Modified On: 2007-6-9
//:: Modified By: riddler
//:: Changed so +2 per lvl for healing circle/natures balance, no cap
void spellsCure(int nDamage, int nMaxExtraDamage, int nMaximized, int vfx_impactHurt, int vfx_impactHeal, int nSpellID)
{
    ...
    if ((nSpellID == SPELL_HEALING_CIRCLE) || (nSpellID == SPELL_NATURES_BALANCE))
    {
...
//edited by riddler 2007-2-6 to prevent auras from effecting mantle
int MyResistSpell(object oCaster, object oTarget, float fDelay = 0.0)
{
    if (fDelay > 0.5)
    {
    ...
    else if(nResist == 3 && (spellID != SPELL_BATTLETIDE || spellID != SPELL_PRAYER || spellID != SPELL_BANE)) //Spell Mantle
    {
        if (fDelay > 0.5)
        {
            fDelay = fDelay - 0.1;
...
void TrapDoElectricalDamage(int ngDamageMaster, int nSaveDC, int nSecondary, object oCreator)
{
    ...
    // IN RANGE CHECK
    if ( GetHitDice(oTarget)<(GetHitDice(oCreator)-5) ||  GetHitDice(oTarget)> (GetHitDice(oCreator)+5))
    {
        nDamage = 0;  //do not damage people out of range
    }
    // nSaveDC lower for low levels, gets higher only if the creator is high and the target is high
    if (GetHitDice(oTarget)*2  < nSaveDC)
    {
        TempnSaveDC =  GetHitDice(oTarget) + d2(GetHitDice(oTarget)/2);
    }
    else TempnSaveDC =   nSaveDC;
    if(!MySavingThrow(SAVING_THROW_REFLEX, oTarget, TempnSaveDC, SAVING_THROW_TYPE_TRAP))
    {
        if (GetHasFeat(FEAT_IMPROVED_EVASION, oTarget))
        {
            nDamage /= 2;
        }
        //failed the safe, nSafe = 0
    }
    else // save succeed!
    {
        nSafe = 1;
    ...
    if (nDamage > 0)
    {
        ...
        if ( ( GetIsDead(oTarget) || GetCurrentHitPoints(oTarget) <= 0 ) && ( GetHitDice(oTarget)>=(GetHitDice(oCreator)-5) && GetHitDice(oTarget)<= (GetHitDice(oCreator)+5) ) ) SetLocalObject(oTarget,"sKiller",oCreator);
    }

    if (!nSafe)// damage is zero (they are out of range) and they failed the save
    {
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,EffectSlow(), oTarget, 14.0f);
        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,EffectVisualEffect(VFX_IMP_LIGHTNING_S),oTarget, 14.0f);
    }
    //Reset the damage;
    ...
    {
        //AW edited not to hurt teammates //
        if(!GetIsReactionTypeFriendly(o2ndTarget,oCreator) && !GetIsFriend(o2ndTarget,oCreator) )
        {
             //IF IN RANGE
             if (  GetHitDice(o2ndTarget) >= (GetHitDice(oCreator)-5) && GetHitDice(o2ndTarget) <= (GetHitDice(oCreator)+5))
             {
                ...
                if(o2ndTarget != oTarget)
                {
                    ...
                    //Adjust the trap damage based on the feats of the target
                    if(!MySavingThrow(SAVING_THROW_REFLEX, o2ndTarget, TempnSaveDC, SAVING_THROW_TYPE_ELECTRICITY))
                    {
                    ...
                    else //save succeed
                    {
                        nSafe = 1;
                        nDamage /= 2;
                        if (GetHasFeat(FEAT_EVASION, o2ndTarget) || GetHasFeat(FEAT_IMPROVED_EVASION, o2ndTarget))
                        {
                             nDamage = 0;
                         }
                    }
            ...
            if (!nSafe)
            {
                 ApplyEffectToObject(DURATION_TYPE_TEMPORARY,EffectSlow(), o2ndTarget, 10.0f);
                 ApplyEffectToObject(DURATION_TYPE_TEMPORARY,EffectVisualEffect(VFX_IMP_LIGHTNING_S),o2ndTarget, 10.0f);
            }
