...
int iRand = Random(80) + Random(80) + 40 + iWeaponSkill + iArmorSkill;
if(iRand > 198)
    {
    iRand = 198;
    }
...
if(iRand < 100)
    {
    CreateItemOnObject("mw_bullets", oPC, iRand);
    }else
        {
        CreateItemOnObject("mw_bullets", oPC, 99);
        CreateItemOnObject("mw_bullets", oPC, iRand - 99);
        }
...
if(iRand < 100)
    {
    CreateItemOnObject("cr_bullets", oPC, iRand);
    }else
        {
        CreateItemOnObject("cr_bullets", oPC, 99);
        CreateItemOnObject("cr_bullets", oPC, iRand - 99);
        }
SendMessageToPC(oPC, "You have created " + IntToString(iRand) + " ammunition.");
return;
}
