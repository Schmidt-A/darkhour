...
int iRand = Random(30) + Random(30) + 30 + iWeaponSkill + iArmorSkill;
if(iRand > 198)
    {
    iRand = 198;
    }
...
if(iRand < 51)
    {
    CreateItemOnObject("mw_throwing", oPC, iRand);
    }else
        {
        CreateItemOnObject("mw_throwing", oPC, 50);
        CreateItemOnObject("mw_throwing", oPC, iRand - 50);
        }
...
if(iRand < 51)
    {
    CreateItemOnObject("cr_throwing", oPC, iRand);
    }else
        {
        CreateItemOnObject("cr_throwing", oPC, 50);
        CreateItemOnObject("cr_throwing", oPC, iRand - 50);
        }
SendMessageToPC(oPC, "You have created " + IntToString(iRand) + " ammunition.");
return;
}
