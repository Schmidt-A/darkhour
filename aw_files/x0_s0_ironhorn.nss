...
if (nRound >= 999)
{
    if (GetLocalInt(OBJECT_SELF, "Bleat") != TRUE)
    {
    ...
    FloatingTextStringOnCreature("You may bleat again in 60 seconds.", OBJECT_SELF);
    effect eBleat = EffectDamageIncrease(DAMAGE_BONUS_8, DAMAGE_TYPE_DIVINE);
    SetLocalInt(OBJECT_SELF, "Bleat", TRUE);
    DelayCommand(RoundsToSeconds(10), DeleteLocalInt(OBJECT_SELF, "Bleat"));
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBleat, OBJECT_SELF, RoundsToSeconds(10));
    ...
    while(GetIsObjectValid(oAlly))
    {
        if(GetIsFriend(oAlly))
        {
            ...
            ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eBleat, oAlly, RoundsToSeconds(10));
        }
