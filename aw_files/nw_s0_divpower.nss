...
// End of Spell Cast Hook
   ...
   //Antiworld change not to stack Attack bonus for clerics Divine Favor  Divine Favor  Battletide
    if ( GetHasEffect(EFFECT_TYPE_ATTACK_INCREASE, OBJECT_SELF )&& ( GetHasSpellEffect(SPELL_DIVINE_FAVOR,OBJECT_SELF) ||  GetHasSpellEffect(SPELL_BATTLETIDE,OBJECT_SELF)||  GetHasSpellEffect(SPELL_DIVINE_POWER,OBJECT_SELF) ) )
    {
        effect eLoop = GetFirstEffect(OBJECT_SELF);
        int nDone = 0;
        while (GetIsEffectValid(eLoop) && nDone == 0)
        {
            if ( GetEffectType(eLoop) == EFFECT_TYPE_ATTACK_INCREASE && (GetEffectSpellId(eLoop) == SPELL_DIVINE_FAVOR || GetEffectSpellId(eLoop) == SPELL_BATTLETIDE  || GetEffectSpellId(eLoop) == SPELL_DIVINE_POWER)  )
            {
   ...
   effect eLink = EffectLinkEffects(eAttackMod, eDur);
    ...
    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eAttack, oTarget, RoundsToSeconds(nCasterLevel));
