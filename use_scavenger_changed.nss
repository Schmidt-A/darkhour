...
// Aez redid it
// need to reorganize this into sub methods
void SearchStuff(object oUser, int nCount, int nDifficulty, int nTotalChance, object oSearch)
{
	int DEBUG = GetLocalInt(GetModule(), "DEBUG_MODE");
	if (GetIsSkillSuccessful(oUser, SKILL_SEARCH, nDifficulty)) {
		int nRand = Random(nTotalChance);
		int nIterate = 0;
		int nFound = -1;
		int nAccumChance = 0;
		while (nIterate < nCount+1) {
			...
			if (nRand >= nAccumChance && nRand < nAccumChance+=nChance) {
				...
				if (GetName(oFound) == "_") {
					...
					//		int nTotalChance = nTotalChance - nChance;

					// stop that shit from showing up again ideally
					SetLocalInt(oSearch, "RefChance"+IntToString(nCount), 0);
					...
					if (DEBUG) {
						FloatingTextStringOnCreature("RESREF:</c  "+GetLocalString(oSearch, "Resref"+IntToString(nFound)), oUser, FALSE);
					}
	...
	if ((sItem == "scavenger") || (sItem == "Shamrock") || (sItem == "ExtraScavenger")) {
		...
		if ((GetDistanceBetween(oJunk, oUser) <= 8.0) && (GetDistanceBetween(oJunk, oUser) > 0.0)) {
			...
			while (sResref != "") {
				...
				nTotalChance += nRefChance;
			}
			nCount--;
			int nDifficulty = GetLocalInt(oSearch, "DC");
			DelayCommand(5.0, SearchStuff(oUser, nCount, nDifficulty, nTotalChance, oSearch));
