...
void main()
{
    ...
    {
        //changed from SPELL_TARGET_SELECTIVEHOSTILE to SPELL_TARGET_STANDARDHOSTILE
        if(spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, OBJECT_SELF) == TRUE)
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay))
            {
                ...
                int nDmgLvl = 10;
                if(nCasterLvl > 10)
                { nDmgLvl += nCasterLvl-10;}
                nDamage = d6(nDmgLvl);
                //Resolve metamagic
                //Adjust the damage based on the Reflex Save, Evasion and Improved Evasion.
                if(nCasterLvl < 21 || GetHasFeat(FEAT_IMPROVED_EVASION, oTarget))
                {
