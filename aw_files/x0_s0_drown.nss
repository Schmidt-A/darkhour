...
// End of Spell Cast Hook
    ...
    if (nRound >= 999)
    {
    if (GetLocalInt(OBJECT_SELF, "Drown") != TRUE)
    {
                    FloatingTextStringOnCreature("You may use drown again in 60 seconds.", OBJECT_SELF);
                    SetLocalInt(OBJECT_SELF, "Drown", TRUE);
                    DelayCommand(RoundsToSeconds(10), DeleteLocalInt(OBJECT_SELF, "Drown"));
                    nDam = FloatToInt(nDam * 0.5);
                    eDam = EffectDamage(nDam, DAMAGE_TYPE_DIVINE);
