...
int iPCUse=FALSE;//Set TRUE if you want player to use it.
int iRandFar=50;//How far shall projectile go? 120=12 tiles.
int iRandNear=15;//What shall it's shortest range be? 10=1 tile.
...
if (GetLocalInt(OBJECT_SELF,"InUse") == 1)
 ...
 location lFinalTarget = Location(GetArea(OBJECT_SELF),vFinalLocation, 0.0);  // GetTargetLocation
 //Done
...
//
 ...
 DelayCommand(fDelay, SetLocalInt(OBJECT_SELF,"reloading",FALSE));
  DelayCommand(5.0,SetLocalInt(OBJECT_SELF,"InUse",0));
  }
}
