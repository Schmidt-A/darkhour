...
void main()
{
    ...
    int nDC = 10 + (nHD/6); //**it was 4**
    int nDuration = 1 + (nHD/6); //**it was 4**
    ...
    while(GetIsObjectValid(oTarget))
    {
        if(GetIsEnemy(oTarget) && oTarget != OBJECT_SELF)
        {
            ...
            if (nRound >= 999)
            {
                if (GetLocalInt(oTarget, "Howled") != TRUE)
                {
                //Apply the VFX impact and effects
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(2)));
                DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oTarget));
                SetLocalInt(oTarget, "Howled", TRUE);
                DelayCommand(RoundsToSeconds(6), DeleteLocalInt(oTarget, "Howled"));
                }
            }
            //Make a saving throw check
            else if(!/*Will Save*/ MySavingThrow(SAVING_THROW_WILL, oTarget, nDC, SAVING_THROW_TYPE_FEAR))
            {
                //Apply the VFX impact and effects
