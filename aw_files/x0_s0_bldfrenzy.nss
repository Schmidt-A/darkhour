...
if (nRound >= 999)
{
if (GetLocalInt(OBJECT_SELF, "Frenzy") != TRUE)
{
FloatingTextStringOnCreature("You may use this ability again in one minute.", OBJECT_SELF);
SetLocalInt(OBJECT_SELF, "Frenzy", TRUE);
DelayCommand(TurnsToSeconds(1), DeleteLocalInt(OBJECT_SELF, "Frenzy"));
effect eHurt = EffectDamageImmunityDecrease(DAMAGE_TYPE_PIERCING, 10);
effect eDam = EffectDamageIncrease(DAMAGE_BONUS_20, DAMAGE_TYPE_DIVINE);
eDam = EffectLinkEffects(eDam, eHurt);
ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDam, OBJECT_SELF, TurnsToSeconds(1));
}
}
else
{
