...
const int GW_COPY_WEAPON_PROPS_TO_UNARMED = 0;
...
// of what polymorph.2da indicates.
// FALSE uses the polymorph.2da to decide whether to copy
const int WS_ALWAYS_COPY_ITEM_PROPS = FALSE;
...
//   weapon was worn at the time of shifting.
// - Set this to any other value ( eg 0 ) to not copy over anything to claw/bite attacks.
const int WS_COPY_WEAPON_PROPS_TO_UNARMED = 0; // Druids normally never get weapon/glove copied over.
        ...
        if ( GetIsStackingProperty(ip) ) // See if it's a stacking property
        {
            ...
            switch ( GetItemPropertyType(ip) ) // Which type of property is it?
            {
                ...
                case 41: // Saving Throw Bonus specific (fort/reflex/will)
                    //fix for wrong values for reflex and will
                    if (iSubType == SAVING_THROW_WILL) iSubType = SAVING_THROW_REFLEX;
                    else if (iSubType == SAVING_THROW_REFLEX) iSubType = SAVING_THROW_WILL;
                                      ...
                                      object oBootsOld, object oBeltOld, object oHelmetOld, object oShield, object oWeapon, object oHideOld)
{
    ...
    //wisdom AC bonus gone for monks! (Moff addition)
    if (GetHasFeat(FEAT_MONK_AC_BONUS, oPC) == TRUE)
    {
        int nWis = GetAbilityModifier(ABILITY_WISDOM, oPC);
        effect eWis = EffectACDecrease(nWis, AC_NATURAL_BONUS);
